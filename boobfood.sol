...
contract BOOBFOODTOKEN is Context, IERC20, Ownable {
    ...
    address payable public charityAddress = 0x6C6Ad0AB710D0BA84EC037c425b92452616e8270; // Charity
	address payable public marketingAddress = 0xe3da8b11C6e48344Af109537F1f6aDa6576e2363; // Marketing
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        uint256 currentRate =  _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection, uint256 tTransferAmount, uint256 tReflection, uint256 tMarketing, uint256 tLiquidity, uint256 tDonation) = _getValues(tAmount);
        uint256 rMarketing =  tMarketing.mul(currentRate);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rReflection, rMarketing, tReflection, tMarketing, tDonation);
    ...
    function _reflectFee(uint256 rReflection, uint256 rMarketing, uint256 tReflection, uint256 tMarketing, uint256 tDonation) private {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tReflection, uint256 tMarketing, uint256 tLiquidity, uint256 tDonation) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection) = _getRValues(tAmount, tReflection, tMarketing, tLiquidity, tDonation, _getRate());
        return (rAmount, rTransferAmount, rReflection, tTransferAmount, tReflection, tMarketing, tLiquidity, tDonation);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tReflection).sub(tMarketing).sub(tLiquidity).sub(tDonation);
        return (tTransferAmount, tReflection, tMarketing, tLiquidity, tDonation);
    }

    function _getRValues(uint256 tAmount, uint256 tReflection, uint256 tMarketing, uint256 tLiquidity, uint256 tDonation, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rReflection = tReflection.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
		uint256 rDonation = tDonation.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rReflection).sub(rMarketing).sub(rLiquidity).sub(rDonation);
    ...
    function removeAllFee() private {
        if(_reflectionFee == 0 && _marketingFee == 0 && _donationFee == 0 && _liquidityFee == 0) return;
